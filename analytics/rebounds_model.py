from __future__ import annotations

import json
from dataclasses import asdict, dataclass
from datetime import datetime
from statistics import NormalDist
from typing import Any

from analytics.features_rebounds import PregameReboundsFeatures, build_pregame_rebounds_features
from analytics.opportunity_model import (
    PregameOpportunityModelConfig,
    PregameOpportunityProjection,
    project_pregame_opportunity,
)
from ingestion.writer import write_model_signals

MODEL_NAME = "pregame_rebounds_baseline"
MODEL_VERSION = "v1"
MIN_EDGE_TO_RECOMMEND = 1.0
MIN_PROBABILITY_TO_RECOMMEND = 0.54
MIN_CONFIDENCE_TO_RECOMMEND = 0.65


@dataclass(frozen=True, slots=True)
class PregameReboundsModelConfig:
    rebounds_per_minute_weights: tuple[tuple[float, str, str], ...] = (
        (0.50, "season_rebounds_avg", "season_minutes_avg"),
        (0.30, "last10_rebounds_avg", "last10_minutes_avg"),
        (0.20, "last5_rebounds_avg", "last5_minutes_avg"),
    )
    rpm_regression_target: float = 0.16
    rpm_regression_factor: float = 0.90
    rpm_floor: float = 0.08
    rpm_ceiling: float = 0.55
    recent_form_last5_factor: float = 0.16
    recent_form_last10_factor: float = 0.08
    recent_form_clamp: float = 1.8
    reb_pct_delta_factor: float = 4.0
    oreb_share_factor: float = 0.0
    opponent_reb_rate_factor: float = 0.0
    pace_factor: float = 0.03
    opponent_clamp: float = 1.0
    pace_clamp: float = 1.0
    home_bonus: float = 0.0
    back_to_back_penalty: float = 0.60
    rest_bonus_per_day: float = 0.06
    rest_bonus_max_days: int = 2
    context_clamp: float = 0.8
    missing_frontcourt_rotation_bonus: float = 0.0
    opportunity_confidence_weight: float = 0.28
    opportunity_score_weight: float = 0.24
    sample_strength_weight: float = 0.14
    stat_stability_weight: float = 0.12
    form_alignment_weight: float = 0.08
    role_stability_weight: float = 0.08
    minutes_trend_weight: float = 0.06


DEFAULT_CONFIG = PregameReboundsModelConfig()


@dataclass(slots=True)
class PregameReboundsBreakdown:
    base_rebounding: float
    recent_form_adjustment: float
    minutes_adjustment: float
    rebound_rate_adjustment: float
    opponent_adjustment: float
    pace_adjustment: float
    context_adjustment: float
    expected_minutes: float
    expected_usage_pct: float
    rebounds_per_minute: float
    opportunity_score: float
    opportunity_confidence: float
    projected_rebounds: float

    def to_dict(self) -> dict[str, float]:
        return asdict(self)


@dataclass(slots=True)
class PregameReboundsProjection:
    features: PregameReboundsFeatures
    breakdown: PregameReboundsBreakdown
    projected_value: float
    distribution_std: float
    over_probability: float
    under_probability: float
    edge_over: float
    edge_under: float
    confidence: float
    recommended_side: str | None

    def to_signal(self) -> dict[str, Any]:
        return {
            "model_name": MODEL_NAME,
            "model_version": MODEL_VERSION,
            "market_phase": "pregame",
            "sportsbook": "fanduel",
            "game_id": self.features.game_id,
            "player_id": self.features.player_id,
            "player_name": self.features.player_name,
            "stat_type": "rebounds",
            "line": self.features.line,
            "projected_value": self.projected_value,
            "over_probability": self.over_probability,
            "under_probability": self.under_probability,
            "edge_over": self.edge_over,
            "edge_under": self.edge_under,
            "confidence": self.confidence,
            "recommended_side": self.recommended_side,
            "metadata_json": json.dumps({"breakdown": self.breakdown.to_dict()}, sort_keys=True),
            "created_at": datetime.utcnow(),
        }


def _value_or_zero(value: float | None) -> float:
    return float(value) if value is not None else 0.0


def _weighted_average(weighted_values: list[tuple[float, float | None]]) -> float:
    numerator = 0.0
    denominator = 0.0
    for weight, value in weighted_values:
        if value is None:
            continue
        numerator += weight * float(value)
        denominator += weight
    if denominator <= 0:
        return 0.0
    return numerator / denominator


def _safe_rate(total: float | None, minutes: float | None) -> float | None:
    if total is None or minutes is None or minutes <= 0:
        return None
    return total / minutes


def _clamp(value: float, lower: float, upper: float) -> float:
    return max(lower, min(value, upper))


def project_pregame_rebounds(
    features: PregameReboundsFeatures,
    config: PregameReboundsModelConfig = DEFAULT_CONFIG,
    *,
    opportunity_config: PregameOpportunityModelConfig | None = None,
    opportunity_projection: PregameOpportunityProjection | None = None,
) -> PregameReboundsProjection:
    if opportunity_projection is None:
        opportunity_projection = project_pregame_opportunity(
            features,
            config=opportunity_config or PregameOpportunityModelConfig(),
        ) if opportunity_config is not None else project_pregame_opportunity(features)
    opportunity = opportunity_projection.breakdown

    rebounds_per_minute = _weighted_average(
        [
            (weight, _safe_rate(getattr(features, rebounds_attr), getattr(features, minutes_attr)))
            for weight, rebounds_attr, minutes_attr in config.rebounds_per_minute_weights
        ]
    )
    rebounds_per_minute = _clamp(
        config.rpm_regression_target + config.rpm_regression_factor * (rebounds_per_minute - config.rpm_regression_target),
        config.rpm_floor,
        config.rpm_ceiling,
    )

    expected_minutes = opportunity.expected_minutes
    expected_usage_pct = opportunity.expected_usage_pct
    base_rebounding = max(0.0, expected_minutes * rebounds_per_minute)

    minutes_adjustment = 0.0
    if features.season_minutes_avg is not None:
        minutes_adjustment = (expected_minutes - features.season_minutes_avg) * rebounds_per_minute * 0.30

    recent_form_adjustment = 0.0
    if features.last5_rebounds_avg is not None and features.season_rebounds_avg is not None:
        recent_form_adjustment += (features.last5_rebounds_avg - features.season_rebounds_avg) * config.recent_form_last5_factor
    if features.last10_rebounds_avg is not None and features.season_rebounds_avg is not None:
        recent_form_adjustment += (features.last10_rebounds_avg - features.season_rebounds_avg) * config.recent_form_last10_factor
    recent_form_adjustment = _clamp(recent_form_adjustment, -config.recent_form_clamp, config.recent_form_clamp)

    rebound_rate_adjustment = 0.0
    if features.last10_reb_pct is not None and features.season_reb_pct is not None:
        rebound_rate_adjustment += (features.last10_reb_pct - features.season_reb_pct) * config.reb_pct_delta_factor
    if features.last10_oreb_avg is not None and features.last10_rebounds_avg not in (None, 0):
        oreb_share = features.last10_oreb_avg / max(features.last10_rebounds_avg, 1.0)
        rebound_rate_adjustment += (oreb_share - 0.24) * config.oreb_share_factor

    opponent_adjustment = 0.0
    if features.opponent_fg_pct_allowed is not None:
        opponent_adjustment += (0.54 - float(features.opponent_fg_pct_allowed)) * config.opponent_reb_rate_factor
    opponent_adjustment = _clamp(opponent_adjustment, -config.opponent_clamp, config.opponent_clamp)

    expected_game_pace = _weighted_average([(0.50, features.team_pace), (0.50, features.opponent_pace)])
    pace_adjustment = 0.0
    if expected_game_pace and features.league_avg_pace is not None:
        pace_adjustment = (expected_game_pace - features.league_avg_pace) * config.pace_factor
    pace_adjustment = _clamp(pace_adjustment, -config.pace_clamp, config.pace_clamp)

    context_adjustment = 0.0
    if features.is_home:
        context_adjustment += config.home_bonus
    if features.back_to_back:
        context_adjustment -= config.back_to_back_penalty
    if features.days_rest is not None and features.days_rest >= 2:
        context_adjustment += min(features.days_rest - 1, config.rest_bonus_max_days) * config.rest_bonus_per_day
    if features.missing_frontcourt_rotation_piece:
        context_adjustment += config.missing_frontcourt_rotation_bonus * _value_or_zero(features.pregame_context_confidence)
    context_adjustment = _clamp(context_adjustment, -config.context_clamp, config.context_clamp)

    projected_rebounds = max(
        0.0,
        base_rebounding
        + recent_form_adjustment
        + minutes_adjustment
        + rebound_rate_adjustment
        + opponent_adjustment
        + pace_adjustment
        + context_adjustment,
    )

    distribution_std = max(1.8, _value_or_zero(features.last10_rebounds_std), projected_rebounds * 0.22)
    if features.line > 0:
        distribution = NormalDist(mu=projected_rebounds, sigma=distribution_std)
        over_probability = 1.0 - distribution.cdf(features.line)
        under_probability = 1.0 - over_probability
        edge_over = projected_rebounds - features.line
        edge_under = -edge_over
    else:
        over_probability = 0.5
        under_probability = 0.5
        edge_over = 0.0
        edge_under = 0.0

    sample_strength = _clamp(features.sample_size / 10.0, 0.0, 1.0)
    stat_stability = 1.0 - _clamp(_value_or_zero(features.last10_rebounds_std) / 5.0, 0.0, 1.0)
    form_alignment = 1.0 - _clamp(abs(_value_or_zero(features.last5_rebounds_avg) - _value_or_zero(features.season_rebounds_avg)) / 6.0, 0.0, 1.0)
    minutes_trend_stability = 1.0 - _clamp(abs(_value_or_zero(features.last5_minutes_avg) - _value_or_zero(features.last10_minutes_avg)) / 6.0, 0.0, 1.0)

    confidence = _clamp(
        config.opportunity_confidence_weight * opportunity.confidence
        + config.opportunity_score_weight * opportunity.opportunity_score
        + config.sample_strength_weight * sample_strength
        + config.stat_stability_weight * stat_stability
        + config.form_alignment_weight * form_alignment
        + config.role_stability_weight * opportunity.role_stability
        + config.minutes_trend_weight * minutes_trend_stability,
        0.0,
        1.0,
    )

    recommended_side = None
    if features.line > 0:
        if edge_over >= MIN_EDGE_TO_RECOMMEND and over_probability >= MIN_PROBABILITY_TO_RECOMMEND and confidence >= MIN_CONFIDENCE_TO_RECOMMEND:
            recommended_side = "OVER"
        elif edge_under >= MIN_EDGE_TO_RECOMMEND and under_probability >= MIN_PROBABILITY_TO_RECOMMEND and confidence >= MIN_CONFIDENCE_TO_RECOMMEND:
            recommended_side = "UNDER"

    breakdown = PregameReboundsBreakdown(
        base_rebounding=round(base_rebounding, 3),
        recent_form_adjustment=round(recent_form_adjustment, 3),
        minutes_adjustment=round(minutes_adjustment, 3),
        rebound_rate_adjustment=round(rebound_rate_adjustment, 3),
        opponent_adjustment=round(opponent_adjustment, 3),
        pace_adjustment=round(pace_adjustment, 3),
        context_adjustment=round(context_adjustment, 3),
        expected_minutes=round(expected_minutes, 3),
        expected_usage_pct=round(expected_usage_pct, 4),
        rebounds_per_minute=round(rebounds_per_minute, 3),
        opportunity_score=round(opportunity.opportunity_score, 4),
        opportunity_confidence=round(opportunity.confidence, 4),
        projected_rebounds=round(projected_rebounds, 3),
    )
    return PregameReboundsProjection(
        features=features,
        breakdown=breakdown,
        projected_value=round(projected_rebounds, 3),
        distribution_std=round(distribution_std, 3),
        over_probability=round(over_probability, 4),
        under_probability=round(under_probability, 4),
        edge_over=round(edge_over, 3),
        edge_under=round(edge_under, 3),
        confidence=round(confidence, 4),
        recommended_side=recommended_side,
    )


def generate_pregame_rebounds_signals(captured_at: datetime | None = None, limit: int | None = None, persist: bool = False) -> list[dict[str, Any]]:
    features = build_pregame_rebounds_features(captured_at=captured_at, limit=limit)
    signals = [project_pregame_rebounds(feature).to_signal() for feature in features]
    if persist and signals:
        write_model_signals(signals)
    return signals
