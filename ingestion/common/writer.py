...
def _upsert_rows(session: Any, model: Any, rows: list[dict[str, Any]], key_fields: tuple[str, ...]) -> None:
    for row in rows:
        existing = session.execute(
            select(model).where(*(getattr(model, field) == row[field] for field in key_fields))
        ).scalar_one_or_none()
        if existing:
...
def write_shot_chart_details(rows: list[dict[str, Any]]) -> None:
    if not rows:
        return
    ...
    with session_scope() as session:
        _upsert_rows(
            ...
            ShotChartDetail,
            rows,
            ("game_id", "player_id", "period", "minutes_remaining", "seconds_remaining", "loc_x", "loc_y"),
        )

    logger.info("Upserted %s shot chart detail rows", len(rows))


def write_hustle_stats_boxscores(rows: list[dict[str, Any]]) -> None:
    if not rows:
        return

    with session_scope() as session:
        _upsert_rows(session, HustleStatsBoxscore, rows, ("game_id", "player_id"))

    logger.info("Upserted %s hustle boxscore rows", len(rows))


def write_matchup_boxscores(rows: list[dict[str, Any]]) -> None:
    if not rows:
        return

    with session_scope() as session:
        _upsert_rows(session, MatchupBoxscore, rows, ("game_id", "offense_player_id", "defense_player_id"))

    logger.info("Upserted %s matchup boxscore rows", len(rows))


def write_win_probability_entries(rows: list[dict[str, Any]]) -> None:
    if not rows:
        return

    with session_scope() as session:
        _upsert_rows(session, WinProbabilityEntry, rows, ("game_id", "event_num"))

    logger.info("Upserted %s win probability rows", len(rows))


def write_player_clutch_stats(rows: list[dict[str, Any]]) -> None:
    if not rows:
        return

    with session_scope() as session:
        _upsert_rows(session, PlayerClutchStats, rows, ("player_id", "season", "clutch_time", "point_diff"))

    logger.info("Upserted %s player clutch stat rows", len(rows))


def write_player_hustle_stats(rows: list[dict[str, Any]]) -> None:
    if not rows:
        return

    with session_scope() as session:
        _upsert_rows(session, PlayerHustleStats, rows, ("player_id", "season"))

    logger.info("Upserted %s player hustle stat rows", len(rows))


def write_player_play_types(rows: list[dict[str, Any]]) -> None:
    if not rows:
        return

    with session_scope() as session:
        _upsert_rows(session, PlayerPlayType, rows, ("player_id", "season", "play_type", "type_grouping"))

    logger.info("Upserted %s player play type rows", len(rows))


def write_player_tracking_stats(rows: list[dict[str, Any]]) -> None:
    if not rows:
        return

    with session_scope() as session:
        _upsert_rows(session, PlayerTrackingStats, rows, ("player_id", "season", "measure_type"))

    logger.info("Upserted %s player tracking stat rows", len(rows))


def write_player_on_off_stats(rows: list[dict[str, Any]]) -> None:
    if not rows:
        return

    with session_scope() as session:
        _upsert_rows(session, PlayerOnOffStats, rows, ("player_id", "team_id", "season", "court_status"))

    logger.info("Upserted %s player on/off stat rows", len(rows))


def write_player_defensive_tracking(rows: list[dict[str, Any]]) -> None:
    if not rows:
        return

    with session_scope() as session:
        _upsert_rows(session, PlayerDefensiveTracking, rows, ("player_id", "season", "defense_category"))

    logger.info("Upserted %s player defensive tracking rows", len(rows))


def write_player_shot_location_stats(rows: list[dict[str, Any]]) -> None:
    if not rows:
        return

    with session_scope() as session:
        _upsert_rows(session, PlayerShotLocationStats, rows, ("player_id", "season"))

    logger.info("Upserted %s player shot location rows", len(rows))


def write_lineup_stats(rows: list[dict[str, Any]]) -> None:
    if not rows:
        return

    with session_scope() as session:
        _upsert_rows(session, LineupStats, rows, ("group_id", "season"))

    logger.info("Upserted %s lineup stat rows", len(rows))
