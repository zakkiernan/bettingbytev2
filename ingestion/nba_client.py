...
NBA_GAME_ROTATION_PASSES = int(os.getenv("NBA_GAME_ROTATION_PASSES", "2"))
...
def _normalize_game_rotation_result_sets(result_sets: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for result_set in result_sets:
        team_rows = _result_set_to_dicts(result_set)
        team_rows.sort(key=lambda row: (
            str(row.get("TEAM_ID") or ""),
            str(row.get("PERSON_ID") or ""),
            _to_float(row.get("IN_TIME_REAL")) or 0.0,
            _to_float(row.get("OUT_TIME_REAL")) or 0.0,
        ))
        stint_numbers: dict[tuple[str, str], int] = {}
        for row in team_rows:
            team_id = str(row.get("TEAM_ID"))
            player_id = str(row.get("PERSON_ID"))
            key = (team_id, player_id)
            stint_numbers[key] = stint_numbers.get(key, 0) + 1
            team_metadata = team_lookup.get(team_id, {})
            in_time_real = _to_float(row.get("IN_TIME_REAL"))
            out_time_real = _to_float(row.get("OUT_TIME_REAL"))
            shift_duration_real = None
            if in_time_real is not None and out_time_real is not None:
                shift_duration_real = max(out_time_real - in_time_real, 0.0)
            rotations.append(
                {
                    "game_id": str(row.get("GAME_ID")),
                    "team_id": team_id,
                    "team_abbreviation": row.get("TEAM_ABBREVIATION") or team_metadata.get("abbreviation"),
                    "team_name": f"{row.get('TEAM_CITY', '')} {row.get('TEAM_NAME', '')}".strip() or team_metadata.get("full_name"),
                    "player_id": player_id,
                    "player_name": f"{row.get('PLAYER_FIRST', '')} {row.get('PLAYER_LAST', '')}".strip(),
                    "stint_number": stint_numbers[key],
                    "in_time_real": in_time_real,
                    "out_time_real": out_time_real,
                    "shift_duration_real": shift_duration_real,
                    "player_points": _to_float(row.get("PLAYER_PTS")),
                    "point_differential": _to_float(row.get("PT_DIFF")),
...
def _summarize_game_rotation(rotations: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for (game_id, team_id), rows in team_groups.items():
        max_out_time_real = max((row["out_time_real"] for row in rows if row.get("out_time_real") is not None), default=None)
        player_groups: dict[str, list[dict[str, Any]]] = {}
        for row in rows:
            player_groups.setdefault(row["player_id"], []).append(row)

        starters = 0
        closers = 0
        total_shift_duration_real = 0.0
        for player_rows in player_groups.values():
            player_rows.sort(key=lambda item: item.get("stint_number", 0))
            first_in_time_real = min((row["in_time_real"] for row in player_rows if row.get("in_time_real") is not None), default=None)
            last_out_time_real = max((row["out_time_real"] for row in player_rows if row.get("out_time_real") is not None), default=None)
            started = first_in_time_real is not None and first_in_time_real <= 0.1
            closed_game = (
                max_out_time_real is not None
                and last_out_time_real is not None
                and last_out_time_real >= max_out_time_real - 0.1
            )
            if started:
                starters += 1
            if closed_game:
                closers += 1
            total_player_shift_duration_real = sum(row.get("shift_duration_real") or 0.0 for row in player_rows)
            total_shift_duration_real += total_player_shift_duration_real
            player_summaries.append(
                {
                    "game_id": game_id,
                    "team_id": team_id,
                    "team_abbreviation": player_rows[0].get("team_abbreviation"),
                    "team_name": player_rows[0].get("team_name"),
                    ...
                    "stint_count": len(player_rows),
                    "first_in_time_real": first_in_time_real,
                    "last_out_time_real": last_out_time_real,
                    "total_shift_duration_real": total_player_shift_duration_real,
                    "avg_shift_duration_real": (total_player_shift_duration_real / len(player_rows)) if player_rows else None,
                }
            )
        ...
        team_summaries.append(
            {
                ...
                "rotation_player_count": len(player_groups),
                "starter_count": starters,
                "closing_count": closers,
                "total_stints": len(rows),
                "max_out_time_real": max_out_time_real,
                "total_shift_duration_real": total_shift_duration_real,
            }
        )
